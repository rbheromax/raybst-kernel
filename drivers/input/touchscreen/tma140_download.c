...
// =========================================================================*/
void
LoadArrayWithSecurityData(unsigned char bStart, unsigned char bLength,
								unsigned char bType)
{
...
signed char fLoadSecurityData(unsigned char bBankNum)
{
    ...
    /*PTJ: 0x1B (00 01 10 11) is more interesting security data than 0x00
...
unsigned char  bTargetStatus; /* bTargetStatus[10];	*/
...
*/
	...
	/* Set SCLK to high Z so there is no clock and wait for a high to
	low transition on SDAT. SCLK is not needed this time. */
	SetSCLKHiZ();
...
*/
	...
	The timing spec that requires that the first Init-Vector happen within
	1 msec after the reset/power up. For this reason, it is not advisable
	to separate the above RESET_MODE or POWER_CYCLE_MODE code
	from the Init-Vector instructions below. Doing so could introduce
	excess delay and cause the target device to exit ISSP Mode.*/
...
unsigned int  iChecksumData = 0;
    ...
    // In this section, a 128-Byte array was specified by #define, so the
    // entire 128-Bytes are written in this loop.*/
    bTargetAddress = 0x00;
    bTargetDataPtr = 0x00;
...
// ==========================================================*/
signed
char fProgramTargetBlock(unsigned char bBankNumber, unsigned char bBlockNumber)
{
	/* printk( "[TSP] %s, %d\n", __func__, __LINE__); */
...
unsigned int load_tma140_frimware_data(int HW_ver)
{
	...
	else if(tsp_special_update == 1)/*special firmware update from t-flash*/
	{
		...
		fd = sys_open("/mnt/sdcard/ray_tsp.hex",
...
int cypress_update(int HW_ver)
{
	...
	fIsError = load_tma140_frimware_data(HW_ver);
	if (fIsError != 0)
    {
        ErrorTrap(fIsError);
		return fIsError;
...
*/
        ...
        /*========================================================*/
        /* Program Flash blocks with predetermined data. In the final
        // application this data should come from the HEX output of
        // PSoC Designer.*/
	...
	UART_PutCRLF(0);
        ...
        /*=======================================================*/
        /*PTJ: Doing Verify
        //PTJ: this code isnt needed in the program flow because we use
        //PROGRAM-AND-VERIFY (ProgramAndVerify SROM Func)
...
*/
	...
	//Block-Verified, andDevice-Checksum Verified. */
