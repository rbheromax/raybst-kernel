...
#define MSM_GPIO_PAGE_0          MSM_GPIO1_REG(0x20)
#define MSM_GPIO_PAGE_1          MSM_GPIO2_REG(0x10)
#define MSM_GPIO_PAGE_2          MSM_GPIO1_REG(0x20)
#define MSM_GPIO_PAGE_3          MSM_GPIO1_REG(0x20)
#define MSM_GPIO_PAGE_4          MSM_GPIO1_REG(0x20)
#define MSM_GPIO_PAGE_5          MSM_GPIO1_REG(0x20)
#define MSM_GPIO_PAGE_6          MSM_GPIO1_REG(0x20)
#define MSM_GPIO_PAGE_7          MSM_GPIO1_REG(0x20)

#define MSM_GPIO_CFG_0          MSM_GPIO1_REG(0x24)
#define MSM_GPIO_CFG_1          MSM_GPIO2_REG(0x14)
#define MSM_GPIO_CFG_2          MSM_GPIO1_REG(0x24)
#define MSM_GPIO_CFG_3          MSM_GPIO1_REG(0x24)
#define MSM_GPIO_CFG_4          MSM_GPIO1_REG(0x24)
#define MSM_GPIO_CFG_5          MSM_GPIO1_REG(0x24)
#define MSM_GPIO_CFG_6          MSM_GPIO1_REG(0x24)
#define MSM_GPIO_CFG_7          MSM_GPIO1_REG(0x24)
...
#define MSM_GPIO_BANK(bank, first, last)				\
	{								\
		.regs = {						\
			...
			.gpio_page =   MSM_GPIO_PAGE_##bank,		\
			.gpio_cfg =    MSM_GPIO_CFG_##bank,		\
		},							\
		.chip = {						\
			.base = (first),				\
			.ngpio = (last) - (first) + 1,			\
...
static void msm_gpio_show_suspend_state_all(void)
{
	unsigned i, j, offset, size = 0;
	unsigned int gpio_cfg;
	struct msm_gpio_chip *chips;

	for (j = 0; j < ARRAY_SIZE(msm_gpio_chips); j++) {
		chips = &msm_gpio_chips[j];
		for (i = chips->chip.base;
			i < chips->chip.base + chips->chip.ngpio; i++) {
			offset = i - chips->chip.base;
			__raw_writel(i, chips->regs.gpio_page);
			gpio_cfg = __raw_readl(chips->regs.gpio_cfg);
			if (__raw_readl(chips->regs.oe) & BIT(offset)) {
				pr_info("gpio_suspend:gpio-%03d-%s-%s-GPIO_PULL:%02X-Function:%02X\n",
						i, "out",
						(__raw_readl(chips->regs.out) &
						BIT(offset)) ? "high" : "low ",
						(gpio_cfg & 0x03),
...
void msm_gpio_enter_sleep(int from_idle)
{
	...
	/*add gpio debug log for sleep current issue*/
	if (msm_gpio_debug_mask & GPIO_DEBUG_SLEEP2)
		msm_gpio_show_suspend_state_all();
}
