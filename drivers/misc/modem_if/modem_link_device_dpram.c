...
dpram_download(struct dpram_link_device *dpld, const char *buf, int len)
{
	...
	while (len > 0) {
		...
		pr_info("[GOTA] <%s> Start write frame %d/%d\n",
...
dpram_upload(struct dpram_link_device *dpld, struct dpram_firmware *uploaddata)
{
	...
	dpram_write_command(dpld, CMD_IMG_SEND_REQ);
	pr_info("[LNK] <%s> write CMD_IMG_SEND_REQ(0x9400)\n", __func__);
	...
	while (1) {
		...
		pr_info("[LNK] <%s> total frame:%d, current frame:%d, data len:%d\n",
	...
	pr_info("[LNK] <%s> 1st dump region data size=%d\n",
		__func__, uploaddata->size);
	pr_info("[LNK] <%s> 2st dump region data size=%d\n",
	...
	pr_info("[LNK] <%s> write CMD_UL_RECEIVE_DONE_RESP(0x9801)\n",
		__func__);
...
{
	pr_info("[LNK] <%s> Received 0xc8 (Boot OK)\n", __func__);
...
static void gota_cmd_handler(struct dpram_link_device *dpld, u16 cmd)
{
	...
	case GOTA_CMD_IMAGE_SEND_RESP:
		pr_info("[GOTA] Send CP-->AP IMAGE_SEND_RESP\n");
