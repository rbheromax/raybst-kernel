		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	unsigned int index_led_red;
	unsigned int index_led_blue;
	unsigned int onMS = 0 , offMS = 0 , onMS_cnt = 0, offMs_cnt = 0 ;
	char onMs_temp[size], offMs_temp[size];
	...
	for (i = 0; i < size; i++) {
		...
		if (buf[i] != ' ' && blank == 1 && buf[i] != '\n') {
			onMs_temp[onMS_cnt] = buf[i];
			onMS_cnt++;
		}
		if (buf[i] != ' ' && blank == 2 && buf[i] != '\n') {
			offMs_temp[offMs_cnt] = buf[i];
			offMs_cnt++;
		}
	}
	if (onMs_temp[0] == '0') {
		...
		int j = 0;
		for (i = 0; i < onMS_cnt; i++) {
			int acc_cnt = onMs_temp[i]-'0';
			for (j = onMS_cnt-1; j > 0; j--)
				acc_cnt *= 10;
	...
	if (offMs_temp[0] == '0')
		...
		for (i = 0 ; i < offMs_cnt ; i++) {
			int acc_cnt = offMs_temp[i]-'0';
			for (j = offMs_cnt-1 ; j > 0 ; j--)
				acc_cnt *= 10;
	...
	if (onMS || offMS) {
		...
		pmic8058_led_set(&led->cdev, LED_OFF);
		led = &led_data[index_led_blue];
		led->cdev.blink_delay_on = onMS;
		led->cdev.blink_delay_off = offMS;
		pmic8058_led_set(&led->cdev, LED_HALF);
	} else {
		pr_info("LED Turn off\n");
		led = &led_data[index_led_red];
