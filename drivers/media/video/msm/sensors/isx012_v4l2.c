...
static int isx012_set_init_mode(void)
{
	...
	isx012_ctrl->settings.preview_size_idx = PREVIEW_SIZE_VGA;
	isx012_ctrl->settings.focus_status = 0;
...
#if 0
	isx012_exif->shutterspeed = 1500000 /
...
{
	cam_info(" BR : %d", isx012_ctrl->settings.brightness);
...
static int isx012_get_LowLightCondition(void)
{
	...
	if (isx012_ctrl->settings.iso == CAMERA_ISO_MODE_AUTO) {/*Auto ISO*/
		...
		cam_info(" ISO : %d", isx012_ctrl->settings.iso);
...
{
	cam_info(" %d -> %d", isx012_ctrl->settings.preview_size_idx, index);
...
{
	CAM_DEBUG(" %d", mode);
	CAM_DEBUG(" focus_status : %d, mode : %d", \
		isx012_ctrl->settings.focus_status, mode);
...
static int32_t isx012_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl, \
				int update_type, int rt)
{
	...
	case MSM_SENSOR_UPDATE_PERIODIC:
		...
		/*	v4l2_subdev_notify(&isx012_ctrl->s_ctrl->
					sensor_v4l2_subdev,
		...
		if (!config_csi2) {
			isx012_ctrl->s_ctrl->curr_csic_params =
			    isx012_ctrl->s_ctrl->csic_params[0];
		...
		v4l2_subdev_notify(&isx012_ctrl->s_ctrl->
			sensor_v4l2_subdev,
			NOTIFY_PCLK_CHANGE,
			&isx012_ctrl->s_ctrl->sensordata->
			pdata->ioclk.vfe_clk_rate);
...
static int isx012_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (temp == 1)
		cam_info(" CAM_5M_ISP_INIT : %d", temp);
	...
	if (err == -EIO) {
		cam_err("isx012_mode_transtion_OM failed\n");
		goto FAIL_END;
	}
	...
	err = isx012_set_init_mode();
	if (err == -EIO) {
		cam_err("isx012_set_init_mode failed!!");
		goto FAIL_END;
	}
...
static int isx012_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	isx012_set_flash(FLASH_OFF);
	isx012_ctrl->flash_mode = CAMERA_FLASH_OFF;
	...
	/* CAM_AF_3.0V Set Low*/
	ret = vreg_disable(vreg_L16);
	if (ret)
		cam_err("error disabling regulator CAM_AF_3.0V\n");
	mdelay(1);
			...
			unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_SENSOR_CSID_INFO:
		...
		CAM_DEBUG("is_csic = %d", csi_info->is_csic);
	...
	void __user *argp)
{
	...
	case CFG_SENSOR_INIT:
		if (config_csi2 == 0) {
			rc = isx012_sensor_setting(s_ctrl,
...
static struct msm_camera_csi_params isx012_csic_params = {
...
static struct msm_camera_csi_params *isx012_csic_params_array[] = {
	&isx012_csic_params,
	&isx012_csic_params,
};

static struct msm_camera_csid_vc_cfg isx012_cid_cfg[] = {
	{0, CSI_YUV422_8, CSI_DECODE_8BIT},
	{1, CSI_EMBED_DATA, CSI_DECODE_8BIT},
	{2, CSI_RESERVED_DATA_0, CSI_DECODE_8BIT},
};

static struct msm_camera_csi2_params isx012_csi_params = {
	.csid_params = {
		.lane_cnt = 2,
		.lut_params = {
			.num_cid = ARRAY_SIZE(isx012_cid_cfg),
			.vc_cfg = isx012_cid_cfg,
		},
	},
	.csiphy_params = {
		.lane_cnt = 2,
...
static struct msm_camera_csi2_params *isx012_csi_params_array[] = {
	&isx012_csi_params,
	&isx012_csi_params,
};
...
/*static struct sensor_calib_data isx012_calib_data;*/
...
static struct msm_sensor_fn_t isx012_func_tbl = {
	.sensor_config = isx012_sensor_config, /*msm_sensor_config */
	...
	.sensor_csi_setting = isx012_sensor_setting,
	.sensor_set_sensor_mode = msm_sensor_set_sensor_mode,
	.sensor_mode_init = msm_sensor_mode_init,
	/*.sensor_setting = isx012_sensor_setting,*/
};
...
int rear_sensor_check_vendorID_before(unsigned char *vendorID)
{
	int err = 0;

	err = isx012_sensor_power_up(&isx012_s_ctrl);
	if (!err)
		isx012_i2c_read(0x0043, (unsigned short *)vendorID);

	*vendorID >>= 4;
	CAM_DEBUG(" check vendor id : %d\n", *vendorID);
	return err;
}

int rear_sensor_check_vendorID_done()
{
	return isx012_sensor_power_down(&isx012_s_ctrl);
}
