...
#define YAS530_FILTER_THRESH		(100)
#define YAS532_FILTER_THRESH		(300)
...
struct yas_pcb_adaptive_filter {
	int num;
	int index;
	int filter_len;
	int filter_noise;
	int32_t sequence[YAS_PCB_MAG_MAX_FILTER_LEN];
};

struct yas_pcb_thresh_filter {
	int32_t threshold;
	int32_t last;
};
#endif
...
#ifdef YAS_PCB_FILTER_ENABLE
int filter_enable = 1;
int filter_len = YAS_PCB_MAG_DEFAULT_FILTER_LEN;
int filter_thresh/* = YAS_PCB_MAG_DEFAULT_FILTER_THRESH*/;
int filter_noise[3] = {YAS_PCB_MAG_DEFAULT_FILTER_NOISE_X,
	YAS_PCB_MAG_DEFAULT_FILTER_NOISE_Y, YAS_PCB_MAG_DEFAULT_FILTER_NOISE_Z};
struct yas_pcb_adaptive_filter adaptive_filter[3];
struct yas_pcb_thresh_filter threshold_filter[3];
#endif

/* functions */
static int Ms3AxesLibSqrt(int, unsigned short *);
static int Ms3AxesLibAtan8(int, int, short *);
static int Ms3AxesLibDir8(int, int, unsigned short *);
#ifdef YAS_PCB_FILTER_ENABLE
static int32_t yas_pcb_square(int32_t);
static void yas_pcb_adaptive_filter_init(struct yas_pcb_adaptive_filter *,
		int, int);
static int32_t yas_pcb_adaptive_filter_filter(struct yas_pcb_adaptive_filter *,
		int32_t);
static void yas_pcb_thresh_filter_init(struct yas_pcb_thresh_filter *, int);
static int32_t yas_pcb_thresh_filter_filter(struct yas_pcb_thresh_filter *,
		int32_t);
#endif
static void yas_pcb_filter_init(void);
static void yas_pcb_filter_filter(int32_t *, int32_t *);
...
static int yas_pcb_read_cal(uint8_t *, int);
...
static int yas_pcb_measure_noise(struct yas_pcb_vector *);
...
static int yas_pcb_test8(int *, int *, int *);
#endif

static int Ms3AxesLibSqrt(int v, unsigned short *ans)
{
	unsigned int min = 0;
	unsigned int max = 46341;
	unsigned int tmp_ans = max >> 1;
	unsigned int ans2;
	signed char num;
	...
	for (num = 0; num < 16; num++) {
		...
		ans2 = tmp_ans * tmp_ans;

		if (ans2 <= (unsigned int)v)
			min = tmp_ans;
		else
			max = tmp_ans;
...
#ifdef YAS_PCB_FILTER_ENABLE
static int32_t
yas_pcb_square(int32_t data)
{
	return data * data;
}

static void
yas_pcb_adaptive_filter_init(struct yas_pcb_adaptive_filter *adap_filter,
		int len, int noise)
{
	int i;
...
static int32_t
yas_pcb_adaptive_filter_filter(struct yas_pcb_adaptive_filter *adap_filter,
		int32_t in)
{
	int32_t avg, sum;
	int i;
	...
	sum = 0;
	for (i = 0; i < adap_filter->filter_len; i++)
		sum += yas_pcb_square(avg - adap_filter->sequence[i]);
...
static void
yas_pcb_thresh_filter_init(struct yas_pcb_thresh_filter *thresh_filter,
		int threshold)
{
...
static int32_t
yas_pcb_thresh_filter_filter(struct yas_pcb_thresh_filter *thresh_filter,
		int32_t in)
{
...
static void
yas_pcb_filter_init(void)
{
	int i;

	for (i = 0; i < 3; i++) {
		yas_pcb_adaptive_filter_init(&adaptive_filter[i], filter_len,
				filter_noise[i]);
		yas_pcb_thresh_filter_init(&threshold_filter[i], filter_thresh);
	}
}

static void
yas_pcb_filter_filter(int32_t *orig, int32_t *filtered)
{
	int i;

	for (i = 0; i < 3; i++) {
		filtered[i]
			= yas_pcb_adaptive_filter_filter(&adaptive_filter[i],
					orig[i]);
		filtered[i] = yas_pcb_thresh_filter_filter(&threshold_filter[i],
				filtered[i]);
	}
}
#else
...
static int
yas_pcb_read_cal(uint8_t *pu08Buf, int size)
{
	int i;
	int ret;
			...
			uint8_t u08Command, uint8_t u08CheckIni)
{
	...
	ret = g_callback.i2c_write(YAS_PCB_ADDR_SLAVE,
			YAS_PCB_ADDR_MEASURE_COMMAND, &u08Command, 1);
	if (0 != ret)
		return YAS_PCB_ERROR_I2C;
...
static void
yas530_calc_magnetic_field(struct yas_pcb_vector *pstXyz, int32_t s32Coef)
{
	int32_t s32Ox;
	int32_t s32Oy1;
	int32_t s32Oy2;
	...
	s32Ox  = gstXy1y2.v[0] - s32HTbl[gstCorrect.s32Fx]
		+ (gs08HardOffset[0] - gstCorrect.s32Rx) * s32Coef;
	s32Oy1 = gstXy1y2.v[1] - s32HTbl[gstCorrect.s32Fy1]
		+ (gs08HardOffset[1] - gstCorrect.s32Ry1) * s32Coef;
	s32Oy2 = gstXy1y2.v[2] - s32HTbl[gstCorrect.s32Fy2]
		+ (gs08HardOffset[2] - gstCorrect.s32Ry2) * s32Coef;
	s32Oy  = s32Oy1 - s32Oy2;
	s32Oz  = -s32Oy1 - s32Oy2;

	pstXyz->v[0] = (gstCorrect.s32K
		* ((100 * s32Ox) + (gstCorrect.s32A2 * s32Oy)
		+ (gstCorrect.s32A3 * s32Oz))) / 10;
	pstXyz->v[1] = (gstCorrect.s32K
		* ((gstCorrect.s32A4 * s32Ox) + (gstCorrect.s32A5 * s32Oy)
		+ (gstCorrect.s32A6 * s32Oz))) / 10;
	pstXyz->v[2] = (gstCorrect.s32K
		* ((gstCorrect.s32A7 * s32Ox) + (gstCorrect.s32A8 * s32Oy)
		+ (gstCorrect.s32A9 * s32Oz))) / 10;
}

static void
yas532_calc_magnetic_field(struct yas_pcb_vector *pstXyz,
...
static int
yas_pcb_calc_magnetic_field(struct yas_pcb_vector *pstXyz)
{
	int32_t s32Coef;
	...
	if (YAS532_DEVICE_ID == gu08DevId) {
		...
		yas532_calc_magnetic_field(pstXyz, YAS532_COEFX_VERSION_AC,
		...
		/* calculate Ohx/y/z[nT] */
		yas530_calc_magnetic_field(pstXyz, s32Coef);
	}
...
static int
yas_pcb_measure_noise(struct yas_pcb_vector *pstXyz)
{
	int nRet;
#if 1
	int temperature;
	...
	nRet = yas_pcb_measure(&stXy1y2, &temperature,
			YAS_PCB_MEASURE_COMMAND_START, YAS_PCB_INT_NOTCHECK);
	if (YAS_PCB_NO_ERROR == nRet) {
		yas_pcb_calc_position(&stXy1y2, pstXyz, temperature);
		nRet = yas_pcb_is_flow_occued(&stXy1y2);
	}
#else
	nRet = YAS_PCB_NO_ERROR;
	pstXyz->v[0] += 100;
	pstXyz->v[1] += 100;
	pstXyz->v[2] += 100;
#endif
...
yas_pcb_test3(void)
{
	...
	if (YAS532_DEVICE_ID == gu08DevId) {
		...
		result = yas_pcb_read_cal(pu08Buf, YAS532_CAL_REG_NUM);
		if (YAS_PCB_NO_ERROR == result)
			yas532_calc_correction(pu08Buf);
	} else {
		gs32Center = YAS530_RAWDATA_CENTER;
		gs32Overflow = YAS530_RAWDATA_OVERFLOW;
		result = yas_pcb_read_cal(pu08Buf, YAS530_CAL_REG_NUM);
		if (YAS_PCB_NO_ERROR == result)
...
yas_pcb_test7(int *ohx, int *ohy, int *ohz)
{
	...
	struct yas_pcb_vector stOhxyz;

	if ((NULL != ohx) && (NULL != ohy) && (NULL != ohz)) {
		if (0 != gstCorrect.s32ZFlag) {
			nRet = yas_pcb_calc_magnetic_field(&stOhxyz);
			if (YAS_PCB_NO_ERROR == nRet) {
				/* [nT]->[uT] */
...
static int
yas_pcb_test8(int *nx, int *ny, int *nz)
{
	int nRet = YAS_PCB_ERROR_ARG;
	int i;
	int l;
	int num;
#if (0 < YAS_PCB_NOISE_LEVEL_DUMMY_NUM)
	int d;
#endif
	int noise;
	uint16_t s = 0;
	struct yas_pcb_vector stXyz;
	struct yas_pcb_vector stFilter;
	struct yas_pcb_vector stM;
	struct yas_pcb_vector stMs;

	if ((NULL != nx) && (NULL != ny) && (NULL != nz)) {
#ifdef YAS_PCB_FILTER_ENABLE
		if (YAS532_DEVICE_ID == gu08DevId)
			filter_thresh = YAS532_FILTER_THRESH;
		else
			filter_thresh = YAS530_FILTER_THRESH;
#endif
...
#if (0 < YAS_PCB_NOISE_LEVEL_DUMMY_NUM)
		for (d = 0; d < YAS_PCB_NOISE_LEVEL_DUMMY_NUM; d++) {
			nRet = yas_pcb_measure_noise(&stXyz);
			if (YAS_PCB_NO_ERROR != nRet)
				return nRet;
			yas_pcb_filter_filter(&(stXyz.v[0]), &(stFilter.v[0]));
		}
#endif
#if (1 < YAS_PCB_NOISE_LEVEL_MEASURE_NUM)
		num = YAS_PCB_NOISE_LEVEL_MEASURE_NUM;
		for (l = 0; l < num; l++) {
			nRet = yas_pcb_measure_noise(&stXyz);
			if (YAS_PCB_NO_ERROR != nRet)
				return nRet;
			...
			yas_pcb_filter_filter(&(stXyz.v[0]), &(stFilter.v[0]));
			for (i = 0; i < 3; i++) {
				noise = stFilter.v[i] / 100;
				stM.v[i] += noise;
				stMs.v[i] += noise * noise;
			}
			g_callback.msleep(YAS_PCB_NOISE_LEVEL_MEASURE_DELAY);
		}

		noise = ((num * stMs.v[0]) - (stM.v[0] * stM.v[0]));
		noise /= (num * (num - 1));
		Ms3AxesLibSqrt(noise, &s);
		*nx = (int)s;

		noise = ((num * stMs.v[1]) - (stM.v[1] * stM.v[1]));
		noise /= (num * (num - 1));
		Ms3AxesLibSqrt(noise, &s);
		*ny = (int)s;

		noise = ((num * stMs.v[2]) - (stM.v[2] * stM.v[2]));
		noise /= (num * (num - 1));
		Ms3AxesLibSqrt(noise, &s);
		*nz = (int)s;
#else
...
/* test 8 */
static int
noise_level_check(int *nx, int *ny, int *nz)
{
#ifdef YAS_PCBTEST_EXTRA
	int ret;
